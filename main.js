...
const options = {
  url: 'https://dev76371.service-now.com/',
  username: 'admin',
  password: 't7dlVwzVWEV3'
};
...
function processRequestResults(error, response, body, callback) {
   ...
   */
   let hibernating = Boolean(isHibernating(response));
   ...
   if (error) {
     ...
     console.error('Bar Response Code.');
     callbackError = response;
   } else if (hibernating == true) {
       callbackError = 'Service Now instnace is hibernating';
